...
pub fn insert_user_index(conn: &mut PooledConn, tx:&NFT_data::Transaction, dapp_id: u64){
    ...
    // insert user index
    conn.exec_drop(
        "insert into user_dapp_index (user_id, dapp_id, index_) values(:user_id, :dapp_id, :index_)",
        params! {
            "user_id" => user_id,
            "dapp_id" => dapp_id,
            "index_" => index,
...
pub fn delete_user_index(conn: &mut PooledConn, tx:&NFT_data::Transaction, dapp_id:u64){
    ...
    // delete
    let stmt = conn.prep("delete from user_dapp_index where user_id=:id and dapp_id=:dapp_id and index_=:index").unwrap();
...
pub fn sql_userinfo(conn: &mut PooledConn, user: &NFT_data::AccountIdentifier)->HashMap<u64, Vec<u64>>
{
    ...
    let res: Vec<(u64, u64)> = conn.query(format!("select dapp_id, index_ from user_dapp_index where user_id={}", user_id)).unwrap();
    // let mut tmp: Vec<u64> = Vec::new();
    for r in res {
        // match user_info.get(&r.0) {
        //     None => {user_info.insert(r.0, Vec::new()); }, 
        //     Some(_) => {
        //         let mut tmp = user_info[&r.0].clone();
        //         tmp.push(r.1);
